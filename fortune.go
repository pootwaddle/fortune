...
func main() {
	...
	logFile.WriteString(fmt.Sprintf("<tr><td>yrMod (7)</td><td>%d</td></tr>\n", joke.YrMod))
	...
	logFile.WriteString(fmt.Sprintf("<tr><td>Today is Day+ </td><td>%d</td></tr>\n", int(dayplus.Days(2021, 3, 18, time.Now()))))
	logFile.WriteString(fmt.Sprintf("<tr><td>Beard is Day+ </td><td>%d</td></tr>\n", int(dayplus.Days(2021, 9, 20, time.Now()))))
	logFile.WriteString("</table>\n")
	logFile.Sync()
